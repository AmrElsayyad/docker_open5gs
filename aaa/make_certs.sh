...
#aaa
openssl genrsa -out $1/aaa.key.pem 1024
openssl req -new -batch -out aaa.csr.pem -key $1/aaa.key.pem -subj /CN=aaa.EPC_DOMAIN/C=KO/ST=Seoul/L=Nowon/O=Open5GS/OU=Tests
openssl ca -cert $1/cacert.pem -days 3650 -keyfile cakey.pem -in aaa.csr.pem -out $1/aaa.cert.pem -outdir . -batch
